...
import org.enhydra.shark.corbaclient.workflowadmin.user.actions.*;
   ...
   public synchronized void refresh (boolean mandatoryRefreshing) {
      ...
      for (int i=0; i<existingUsersPanel.getTable().getRowCount(); i++) {
         ...
         try {
            ...
            existingUsersPanel.setColumnValueOfSelectedRow(1,uga.getUserLastName(uname));
            existingUsersPanel.setColumnValueOfSelectedRow(2,uga.getUserEMailAddress(uname));
         }catch (Exception ex) {}
      }
