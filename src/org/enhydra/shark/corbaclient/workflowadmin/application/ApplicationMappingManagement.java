...
import org.enhydra.shark.corbaclient.workflowadmin.application.actions.*;
...
public class ApplicationMappingManagement extends ActionPanel {
   ...
   public synchronized void refresh (boolean mandatoryRefreshing) {
      ...
      try {
         allMappings=new ArrayList(Arrays.asList(mm.getApplicationMappings()));
      } catch (Exception ex) {
      ...
      while (it.hasNext()) {
         ...
         try {
            ...
            if (am.getApplicationMode()!=-Integer.MAX_VALUE) {
               v.add(new Integer(am.getApplicationMode()));
            } else {
               v.add("");
            }
   ...
   private void createApplicationsMap (Collection packageColl) {
      ...
      try {
         ApplicationMap[] maps=mm.getAllApplications();
