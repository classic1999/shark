...
public class EngineTreeModel extends DefaultTreeModel {
   ...
   private void insertManagers (WfProcessMgr[] mgrs) {
      ...
      if (mgrs!=null) {
         for (int i=0; i<mgrs.length; i++) {
            ...
            //try{System.out.println("Inserting manager "+mgrs[i].name()+" for pkgId="+pkgId);}catch(Exception ex){}
   ...
   private void insertManager (String pkgId,WfProcessMgr newMgr) {
      //System.out.println("Seraching for pkg Node for pkgId="+pkgId+", to add mgr"+newMgr);
   ...
   private void updateProcesses () {
      ...
      while (add.hasNext()) {
         ...
         try {
            ...
            //System.out.println("Inserting "+mgr.name()+" - "+pr.name()+" - "+pr.key());
