...
public class DisableDefinition extends ActionBase {
   ...
   public DisableDefinition (ProcessInstantiationManagement pim) {
      super(pim);
   }
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if (userObject instanceof Package) {
         ...
         try {
            WfProcessMgrIterator wpmgi=SharkClient.getCommonExpressionBuilder().getEnabledOrDisabledManagersForPackage(pkg.get("Id").toString(),true);
            WfProcessMgr[] mgrs=wpmgi.get_next_n_sequence(0);
            if (mgrs!=null) {
               for (int i=0; i<mgrs.length; i++) {
                  try {
                     mgrs[i].set_process_mgr_state(process_mgr_stateType.disabled);
         ...
         try {
            ((WfProcessMgr)userObject).set_process_mgr_state(process_mgr_stateType.disabled);
